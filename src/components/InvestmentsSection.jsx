const StatRow = ({ label, value }) => (
  <div className="flex justify-between items-center text-sm border-b border-gray-100 py-3 last:border-0">
    ...
    <span className="text-gray-900 font-medium font-sans text-right ml-4">{value}</span>
  </div>
);
...
const SectionHeader = ({ topLabel, title, isSerif = false }) => (
  <div className="text-center mb-[60px]">
    ...
    <h2 className={`text-4xl md:text-5xl text-gray-900 ${isSerif ? 'font-serif' : 'font-bold font-sans'}`}>
      {title}
    </h2>
  </div>
);
  ...
  const privateFunds = [
    {
      ...
      stats: { class: "Fixed Income", min: "$100k", type: "Qualified Purchaser", redemptions: "Instant" }
    },
    {
      ...
      stats: { class: "U.S. Treasury", min: "$5M", type: "Qualified Purchaser", redemptions: "Instant" }
    },
    {
      ...
      stats: { class: "Senior Credit", min: "$10K", type: "Qualified Purchaser", redemptions: "On-Demand" }
    }
  ];
  ...
  const publicStocks = [
    {
      ...
      stats: { class: "Tokenized Public Stock", type: "Retail", detail: "Secondary Market Trading" }
    },
    {
      ...
      stats: { class: "Tokenized Public Stock", type: "Retail", detail: "Waitlist Active" }
    }
  ];
    ...
    <section className="bg-[#f3f7fc] py-20 md:py-[120px] px-5 md:px-10 space-y-[120px]">
      <div className="max-w-[1160px] mx-auto flex flex-col gap-[120px]">
        ...
        <div className="w-full flex flex-col items-center">
          <SectionHeader topLabel="Private Funds" title="Tokenize Your Portfolio" />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
            {privateFunds.map((fund, idx) => (
              <div key={idx} className="bg-white flex flex-col shadow-sm border border-gray-100">
                <div className="h-[200px] overflow-hidden"><img src={fund.image} className="w-full h-full object-cover" /></div>
                <div className="p-8 flex flex-col gap-5">
                  <div className="flex items-center gap-3 text-[10px] font-bold uppercase text-blue-500">{fund.tag}</div>
                  <h3 className="text-xl font-bold text-gray-900 min-h-[56px] leading-snug">{fund.title}</h3>
                  <p className="text-sm text-gray-500 min-h-[60px]">{fund.desc}</p>
                  <div className="pt-2">
          ...
          <button className="bg-black text-white px-10 py-4 rounded-full font-medium hover:bg-gray-800 transition-all flex items-center gap-2">
        ...
        <div className="w-full flex flex-col items-center">
          <SectionHeader topLabel="For investors" title="Tokenized Alternative Investments" isSerif={true} />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
            <div className="bg-white shadow-sm border border-gray-100">
              <div className="h-[200px] bg-[#1a1a1a] flex items-center justify-center p-10">
                <div className="text-center text-white border border-white/20 p-4">
                   ...
                   <p className="text-[8px] opacity-60">International Hotel & Resort</p>
                   <p className="text-[10px] font-serif italic">Maldives</p>
                </div>
              </div>
              <div className="p-8">
                <h3 className="text-xl font-bold mb-4">WLFI DAR Maldives Limited Tokenized Asset Offering</h3>
                <p className="text-sm text-gray-500 mb-6">An investment opportunity offering economic exposure to the development of the Trump International Hotel & Resort, Maldives.</p>
          ...
          <div className="w-full text-[10px] text-gray-400 leading-relaxed max-w-[900px] text-left self-start">
            <p>
             * DT Marks Defi LLC, an entity affiliated with Donald J. Trump and certain of his family members, has an indirect economic interest through approximately 38% beneficial ownership of an affiliate of WLFI DAR Maldives Limited ("WLFD"), which is entitled to earn a portion of the proceeds from the token issuance by virtue of its indirect economic interest in WLFD. DT Marks Defi LLC and Donald J. Trump are not the issuer of the tokens, are not sponsoring, endorsing, promoting or soliciting this offering, and they have no responsibility for the structure or performance of this token offering. Any references to or quotes or imagery attributed to or associated with Donald J. Trump or his family members should not be construed as an endorsement or representation or warranty with respect to any product or service offering. This token is not political or associated with any political campaign.
...
** This material is intended for informational purposes only. This is not an offer or solicitation to buy or sell securities and is not intended as investment advice or recommendation.
        ...
        {/* --- PUBLIC STOCKS --- */}
        <div className="w-full flex flex-col items-center">
          <SectionHeader topLabel="Public Funds" title="Tokenized Public Stocks" isSerif={true} />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full mb-12">
            {publicStocks.map((stock, idx) => (
              <div key={idx} className="bg-white shadow-sm border border-gray-100">
                <div className="h-[180px] bg-[#030b1e] flex flex-col items-center justify-center p-8 text-center">
                  <h2 className="text-white text-2xl font-bold tracking-tighter mb-2">{stock.name.split(' ')[0]}</h2>
                  <p className="text-white/60 text-[10px] uppercase">{stock.status}</p>
                </div>
                <div className="p-8 flex flex-col gap-5">
                  <p className="text-[10px] font-bold text-blue-500 uppercase tracking-tight">{stock.tag}</p>
                  <h3 className="text-lg font-bold text-gray-900 min-h-[50px]">{stock.name}</h3>
                  <p className="text-sm text-gray-500 min-h-[80px]">{stock.desc}</p>
                  <div className="pt-2">
...
const ComingSoonCard = ({ title = "Coming Soon", isDark = false }) => (
  <div className="bg-white shadow-sm border border-gray-100 opacity-80">
    <div className={`h-[180px] flex items-center justify-center ${isDark ? 'bg-black text-white' : 'bg-gray-900 text-white'}`}>
